    ...
    <nav className="fixed top-0 w-full bg-white/95 backdrop-blur-md z-50 border-b border-red-100 shadow-sm">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-2xl font-bold text-slate-900">ST<span className="text-red-600">Hamburgueria</span></div>
        </div>
        ...
        {/* Botões para Mobile (Menu Hambúrguer + Botão rápido de Pedido) */}
        <div className="flex items-center gap-3 md:hidden">
          <a 
            href={whatsappLink} 
            target="_blank" 
            rel="noopener noreferrer" 
            className="bg-red-600 text-white px-4 py-2 rounded-full font-bold text-xs hover:bg-red-700 transition-all flex items-center gap-1.5"
          >
            <span>📲</span> Pedir
          </a>
      ...
      {/* Menu Gaveta Mobile (Mobile Menu Dropdown) */}
      <div className={`md:hidden transition-all duration-300 ease-in-out border-t border-red-50 bg-white ${isOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-4 pt-2 pb-4 space-y-3 shadow-inner">
          <a 
            ...
            className="block font-medium py-2 px-3 text-slate-700 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all"
          >
            🍔 Cardápio
          </a>
          <a 
