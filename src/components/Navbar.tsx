...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full bg-white/95 backdrop-blur-md z-50 border-b border-red-100 shadow-sm">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-8 py-4 flex justify-between items-center">
        
        {/* Logo — A fonte encolhe dinamicamente de acordo com a largura da tela */}
        <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-red-600 rounded-full flex items-center justify-center text-white font-bold text-base sm:text-xl">
            🍔
          </div>
          <div className="text-[13px] min-[360px]:text-sm min-[400px]:text-base sm:text-2xl font-bold text-slate-900 tracking-tight transition-all duration-200">
        ...
        {/* Botões para Mobile — Equilibrado para caber junto com a fonte menor */}
        <div className="flex items-center gap-1.5 sm:gap-3 md:hidden">
          <a 
            href={whatsappLink} 
            target="_blank" 
            rel="noopener noreferrer" 
            className="bg-red-600 text-white px-3.5 py-2 sm:px-5 sm:py-2.5 rounded-full font-bold text-xs sm:text-sm hover:bg-red-700 active:scale-95 transition-all flex items-center gap-1.5 shadow-sm whitespace-nowrap"
          >
            <span>📲</span> <span>Pedir Agora</span>
          </a>
          
          <button 
            onClick={() => setIsOpen(!isOpen)}
            className="p-1.5 text-slate-700 hover:text-red-600 focus:outline-none rounded-xl hover:bg-slate-50"
