  ...
  return (
    <div className="w-full max-w-3xl px-4 py-8 md:py-20 animate-fade-in">
      <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-2xl shadow-slate-200/50 border border-slate-100">
        ...
        <p className="text-sm md:text-base text-slate-500 font-medium mb-6 md:mb-8">Defina o nome da marca e as cores base para o seu novo site.</p>
        ...
        <form onSubmit={avancarParaEtapa3} className="space-y-6 md:space-y-8">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-3 md:mb-4">Paleta de Cores</label>
            <div className="flex gap-4 md:gap-6 justify-center md:justify-start">
              {[0, 1, 2].map((index) => (
                <div key={index} className="flex flex-col items-center gap-2 md:gap-3">
                  ...
                  <span className="text-[10px] md:text-xs text-slate-400 font-mono font-bold uppercase">{infoSite.cores[index]}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="pt-6 md:pt-8 border-t border-slate-100 flex flex-col-reverse md:flex-row justify-between items-center gap-3 md:gap-0">
            <button type="button" onClick={voltarEtapa} className="w-full md:w-auto px-6 py-3.5 md:py-3 text-slate-500 font-bold hover:text-slate-800 hover:bg-slate-50 rounded-xl transition-colors">
              Voltar
            </button>
            <button type="submit" className="w-full md:w-auto px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-base md:text-lg shadow-lg shadow-indigo-600/30 transition-all hover:-translate-y-1">
              Abrir Construtor →
            </button>
          </div>
        </form>
