  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/40 overflow-hidden p-6 md:p-10 transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 pb-6 border-b border-slate-100 gap-4">
        ...
        <button 
          type="button"
          className="group flex items-center gap-2 text-sm font-bold text-slate-700 hover:text-indigo-700 cursor-pointer transition-all duration-200 bg-slate-50 hover:bg-indigo-50/50 px-4 py-2 rounded-xl border border-slate-200/60 w-fit"
        >
        ...
        {posts.map((post) => (
          ...
          >
            ...
            <div className="aspect-video w-full flex-shrink-0 bg-slate-100 relative overflow-hidden">
              ...
              <span className="absolute top-3 left-3 text-xs font-extrabold text-indigo-800 uppercase tracking-wider bg-white/90 backdrop-blur-md shadow-xs px-2.5 py-1 rounded-md border border-white/20">
                {post.categoria}
              </span>
            </div>
            ...
            <div className="flex-1 p-5 flex flex-col justify-between min-w-0">
              <div>
                {/* Metadados */}
                <div className="flex items-center gap-2 mb-2.5 text-sm text-slate-600 font-semibold">
                  <span>{post.data}</span>
              ...
              <div className="pt-3 border-t border-slate-100/80 flex items-center justify-between text-sm font-bold text-slate-700 group-hover:text-indigo-700 transition-colors">
